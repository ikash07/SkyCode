    ...
    <div className="flex h-full flex-col rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
      <div className="mb-3">
        ...
        <div className="mt-2 flex gap-2">
          <input
            ...
            className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm outline-none placeholder:text-[var(--color-muted)]"
          />
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1">
        {results.length ? results.map((result) => (
          <button key={result.path} onClick={() => onOpenFile(result.path)} className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3 text-left hover:border-[var(--color-accent)]">
